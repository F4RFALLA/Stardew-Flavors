...
class TestesModelReceita(TestCase):
    ...
    def test_media_e_total_de_avaliacoes(self):
        ...
        Comentario.objects.create(
            receita=self.receita,
            autor=segundo_usuario,
            texto='Gostei, mas mudaria alguns ingredientes.',
            nota=3,
        )
...
class TestesModelComentario(TestCase):
    ...
    def test_comentario_com_nota_menor_que_um(self):
        comentario = Comentario(
            receita=self.receita,
            autor=self.usuario,
    ...
    def test_comentario_com_nota_maior_que_cinco(self):
        comentario = Comentario(
            receita=self.receita,
            autor=self.usuario,
            texto='Comentário inválido.',
...
class TestesViewListarReceitas(TestCase):
    ...
    def test_busca_por_titulo(self):
        Receita.objects.create(
            titulo='Bolo Rosa',
            descricao='Um bolo feito com melão.',
            origem='Vila Pelicanos',
            ingredientes='1 melão\n1 farinha',
...
class TestesPermissoesReceita(TestCase):
    ...
    def test_autor_exclui_a_propria_receita(self):
        self.client.force_login(self.autor)
...
class TestesPermissoesComentario(TestCase):
    ...
    def test_autor_acessa_edicao_do_proprio_comentario(self):
    ...
    def test_outro_usuario_nao_edita_comentario(self):
        self.client.force_login(self.outro_usuario)
    ...
    def test_outro_usuario_nao_exclui_comentario(self):
        self.client.force_login(self.outro_usuario)
    ...
    def test_administrador_exclui_qualquer_comentario(self):
        self.client.force_login(self.administrador)
...
class TestesCriacaoComentario(TestCase):
    ...
    def test_usuario_autenticado_cria_comentario(self):
        self.client.force_login(self.usuario)
    ...
    def test_autor_do_comentario_e_definido_automaticamente(self):
        self.client.force_login(self.usuario)
    ...
    def test_receita_do_comentario_e_definida_automaticamente(self):
        self.client.force_login(self.usuario)
    ...
    def test_usuario_deslogado_nao_cria_comentario(self):
        resposta = self.client.post(
            self.url,
            self.dados_validos,
        )
    ...
    def test_comentario_com_nota_invalida_nao_e_criado(self):
        self.client.force_login(self.usuario)

        dados_invalidos = {
            'nota': 6,
            'texto': 'Comentário com nota inválida.',
        }
        ...
        formulario = resposta.context.get(
    ...
    def test_comentario_sem_texto_nao_e_criado(self):
        self.client.force_login(self.usuario)
...
class TestesFavoritos(TestCase):

    def setUp(self):
        self.autor = User.objects.create_user(
            username='autor_favoritos',
            password='senha123',
        )

        self.usuario = User.objects.create_user(
            username='usuario_favoritos',
            password='senha123',
        )

        self.outro_usuario = User.objects.create_user(
            username='outro_usuario_favoritos',
            password='senha123',
        )

        self.receita = Receita.objects.create(
            titulo='Torta de Ruibarbo',
            descricao='Uma torta doce preparada com ruibarbo.',
            origem='Vila Pelicanos',
            ingredientes='1 ruibarbo\n1 farinha\n1 açúcar',
            modo_preparo='Misture tudo e asse.',
            autor=self.autor,
        )

        self.outra_receita = Receita.objects.create(
            titulo='Pudim de Banana',
            descricao='Uma sobremesa feita com banana.',
            origem='Ilha Gengibre',
            ingredientes='1 banana\n1 leite',
            modo_preparo='Misture e cozinhe.',
            autor=self.autor,
        )

        self.url_alternar = reverse(
            'alternar_favorito',
            kwargs={
                'pk': self.receita.pk,
            },
        ...
        self.url_listar = reverse(
            'listar_favoritos',
        )

    def test_usuario_autenticado_favorita_receita(self):
        self.client.force_login(self.usuario)
    ...
    def test_usuario_desfavorita_receita(self):
        Favorito.objects.create(
            usuario=self.usuario,
            receita=self.receita,
        )
    ...
    def test_usuario_deslogado_nao_favorita(self):
        resposta = self.client.post(
            self.url_alternar,
        )

        destino_esperado = (
            f"{reverse('login')}?next={self.url_alternar}"
        )
    ...
    def test_pagina_de_favoritos_exige_login(self):
        resposta = self.client.get(
            self.url_listar,
        )

        destino_esperado = (
            f"{reverse('login')}?next={self.url_listar}"
        )
    ...
    def test_usuario_autenticado_acessa_pagina_de_favoritos(self):
        self.client.force_login(self.usuario)
        ...
        self.assertTemplateUsed(
            resposta,
            'listar_favoritos.html',
        )

    def test_pagina_exibe_receitas_favoritadas(self):
        Favorito.objects.create(
            usuario=self.usuario,
            receita=self.receita,
        )
    ...
    def test_pagina_nao_exibe_receita_nao_favoritada(self):
        Favorito.objects.create(
            usuario=self.usuario,
            receita=self.receita,
        )
    ...
    def test_usuarios_podem_favoritar_a_mesma_receita(self):
        Favorito.objects.create(
            usuario=self.usuario,
            receita=self.receita,
        )
...
class TestesFavoritoNosDetalhes(TestCase):

    def setUp(self):
        self.usuario = User.objects.create_user(
            username='usuario_detalhes_favorito',
            password='senha123',
        )

        self.receita = Receita.objects.create(
            titulo='Prato do Mar',
            descricao='Uma refeição preparada com frutos do mar.',
            origem='Praia',
            ingredientes='1 peixe\n1 camarão',
            modo_preparo='Misture e cozinhe.',
            autor=self.usuario,
    ...
    def test_receita_nao_favoritada_envia_false_ao_template(self):
        self.client.force_login(self.usuario)
    ...
    def test_receita_favoritada_envia_true_ao_template(self):
        Favorito.objects.create(
            usuario=self.usuario,
            receita=self.receita,
        )
